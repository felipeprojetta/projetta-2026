...
const Superficies = (() => {
  ...
  function load() {
    ...
    // Felipe sessao 35: HPL TS Estrutural — mesmo merge-por-descricao idempotente
    // do Aluminio Maciço acima. Respeita tombstones, sem flag. Garante que as
    // cores HPL aparecam em clientes ja existentes (e novas cores HPL no SEED).
    {
      const hplSeed = SEED_SUPERFICIES.filter(s =>
        /^HPL\s+TS\s+ESTRUTURAL/i.test(s.descricao || '')
      ).map(normalize);
      if (hplSeed.length > 0) {
        const normDesc = (s) => String(s || '').toUpperCase().replace(/\s+/g, ' ').trim();
        ...
        const faltantes = hplSeed.filter(s => {
          const d = normDesc(s.descricao);
          return !existentes.has(d) && !tombstones.has(d);
        });
        if (faltantes.length > 0) {
          state.superficies = state.superficies.concat(faltantes);
          store.set('superficies_lista', state.superficies);
          console.log('[Superficies] HPL cores adicionadas: ' + faltantes.length);
        }
      }
    }
    ...
    // Backup do estado pre-migracao em 'superficies_lista__pre_ezy_color'
    // pra reverter em caso de problema.
    //
    // Casos atendidos pela regex:
    //  - "CHAPA ALUMINIO 2.5 MM - MADEIRA - 1500 x 3000"
    //      -> "CHAPA ALUMINIO MACICO 2.5 MM - EZY COLOR - MADEIRA - 1500 x 3000"
    //  - "Chapa Aluminio 2.5 Mm - Prata - Dn01 - 1500 X 6000"
    //      -> "CHAPA ALUMINIO MACICO 2.5 MM - EZY COLOR - PRATA DN01 - 1500 x 6000"
    //  - "CHAPA ALUMINIO MACICO 2.5 MM - PR9005T50 ... T50 (EZYCOLOR) - 1500 x 3000"
    //      -> "CHAPA ALUMINIO MACICO 2.5 MM - EZY COLOR - PR9005T50 ... T50 - 1500 x 3000"
    //  - "Chapa Aluminio Macico 2.5 Mm - ... T50 ( - 1500 X 6000"  (truncado mal)
    //      -> "CHAPA ALUMINIO MACICO 2.5 MM - EZY COLOR - ... T50 - 1500 x 6000"
    //
    // NAO mexe em:
    //  - Cores ja' com WEG no nome (cadastrados sessao 30)
    //  - Cores ja' no padrao novo "- EZY COLOR -"
    //  - Cores que nao batem com a regex (ACM, HPL, vidro etc)
    if (!store.get('ezy_color_v1_migrated_2026_05_14')) {
      const backup = JSON.parse(JSON.stringify(state.superficies));
      store.set('superficies_lista__pre_ezy_color', backup);

      let renomeadas = 0;
      state.superficies.forEach(s => {
        const novo = migrarParaEzyColor(s.descricao);
        if (novo && novo !== s.descricao) {
          s.descricao = novo;
          renomeadas++;
        }
      });
      if (renomeadas > 0) {
        store.set('superficies_lista', state.superficies);
        console.log('[Superficies] EZY COLOR aplicado em ' + renomeadas + ' chapa(s). Backup em "superficies_lista__pre_ezy_color".');
      }
      store.set('ezy_color_v1_migrated_2026_05_14', true);
    }
  ...
  /**
   * Felipe (sessao 30): renomeia uma descricao de chapa de aluminio macico
   * pro padrao novo "CHAPA ALUMINIO MACICO 2.5 MM - EZY COLOR - <COR> - <MEDIDA>".
   *
   * Retorna null se a descricao:
   *   - ja' tem WEG no nome (nova, do catalogo WEG)
   *   - ja' esta no formato novo " - EZY COLOR - " (idempotente)
   *   - nao bate com a regex de chapa de aluminio macico
   *
   * Normalizacoes aplicadas na cor:
   *   - Remove "(EZYCOLOR)" ou "(EZY COLOR)" entre parenteses
   *   - Remove parentesis orfaos no final (caso de truncamento)
   *   - Substitui hifens internos por espacos (ex: "Prata - Dn01" -> "Prata Dn01")
   *   - Uppercase + colapsa multiespacos
   *
   * Normalizacao da medida: lowercase com " x " entre os numeros.
   */
  function migrarParaEzyColor(desc) {
    if (!desc) return null;
    if (/\bWEG\b/i.test(desc)) return null;
    // Só pula se ja' esta no formato NOVO " - EZY COLOR - " (idempotente)
    if (/\s-\s+EZY\s+COLOR\s+-\s/i.test(desc)) return null;
    const re = /^(\s*CHAPA\s+ALUMINIO\s+(?:MACICO\s+)?2[.,]5\s*MM)\s*-\s*(.*?)\s*-\s*(\d+\s*[xX]\s*\d+)\s*$/i;
    const m = String(desc).match(re);
    if (!m) return null;
    let cor = m[2].trim();
    cor = cor.replace(/\s*\(\s*EZY\s*COLOR\s*\)\s*/gi, '');
    cor = cor.replace(/\s*\([^)]*$/, '');
    cor = cor.replace(/\s*\(\s*$/, '');
    cor = cor.replace(/\s*-\s*/g, ' ');
    cor = cor.replace(/\s+/g, ' ').trim().toUpperCase();
    const medida = m[3].toLowerCase().replace(/\s*[xX]\s*/, ' x ');
    return 'CHAPA ALUMINIO MACICO 2.5 MM - EZY COLOR - ' + cor + ' - ' + medida;
  }
