...
const Acessorios = (() => {
  ...
  /**
   * Felipe sessao 35: cadastro do VEDA PORTA para PORTAS INTERNAS.
   * Codigos PA-VED<medida 4 digitos>INT (620, 720, 820, 920, 1020, 1120),
   * fornecedor PRIMA FERRAGENS, familia "Veda Porta", Mod. 230 embutido
  ...
  function migrarVedaPortaInterna() {
    ...
    const NOVOS = [620, 720, 820, 920, 1020, 1120].map(function (m) {
      ...
      return {
        codigo: cod,
        fornecedor: 'PRIMA FERRAGENS',
        descricao: 'VEDA PORTA AUTOMATICO - MOD. 230 - EMBUTIDO PRETO ' + m + ' MM - PORTAS INTERNAS',
        familia: 'Veda Porta',
        preco: PRECOS[cod] || 0,
      };
    });
    let novosAdicionados = 0, precosAtualizados = 0;
    const codigosExistentes = new Set(state.acessorios.map(a => a.codigo));
    NOVOS.forEach(novo => {
      if (!codigosExistentes.has(novo.codigo)) {
        state.acessorios.push(normalize(novo));
        ...
        const ex = state.acessorios.find(a => a.codigo === novo.codigo);
        const pNovo = PRECOS[novo.codigo] || 0;
        if (ex && (Number(ex.preco) || 0) === 0 && pNovo > 0) {
          ex.preco = pNovo;
          precosAtualizados++;
        }
      }
    });
    if (novosAdicionados > 0 || precosAtualizados > 0) {
      console.log('[Acessorios] Veda Porta interna: ' + novosAdicionados
