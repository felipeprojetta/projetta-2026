...
const ChapasPortaExterna = (() => {
  ...
  const TABELA = {
    ...
    12: {
      '1F': [
        ...
        { id: 'tampa_friso_horizontal', label: 'Tampa Friso Horizontal',
          ...
          comp: ctx => ctx.dBFH + ctx.REF,
          ext: 2, int: 2, categoria: 'porta' },
        // Friso horizontal 1F: divide a 1480 da borda quando passa de 1480
        // (largura util da chapa). Pedaco 1 = 1480, pedaco 2 = resto. O FRISO
        // VERTICAL entra no corte (a 1480 da borda) e passa a ser contabilizado.
        { id: 'friso_horizontal', label: 'Friso Horizontal',
          largura: ctx => Math.min(ctx.larguraQuadro1F, 1480),
          comp: ctx => ctx.eF + 100,
          ext: 1, int: 1, categoria: 'porta' },
        { id: 'friso_horizontal_2', label: 'Friso Horizontal 2',
          largura: ctx => Math.max(0, ctx.larguraQuadro1F - 1480),
          comp: ctx => ctx.eF + 100,
          ext: ctx => ctx.larguraQuadro1F > 1480 ? 1 : 0,
          int: ctx => ctx.larguraQuadro1F > 1480 ? 1 : 0, categoria: 'porta' },
        { id: 'friso_vertical', label: 'Friso Vertical',
          largura: ctx => ctx.eF + 100,
          comp: ctx => ctx.alturaQuadro,
          ext: ctx => ctx.larguraQuadro1F > 1480 ? 1 : 0,
      ...
      '2F': [
        ...
        { id: 'tampa_maior_03', label: 'Tampa Maior 03',
          ...
          comp: ctx => ctx.alturaQuadro - 2*ctx.dBFH - 2*ctx.eF + 2*ctx.REF - 1,
          ext: 0, int: 1, categoria: 'porta' },
        // Friso horizontal 2F: divide a 1480 da borda quando passa de 1480.
        // Pedaco 1 = 1480, pedaco 2 = resto. O FRISO VERTICAL entra no corte.
        { id: 'friso_horizontal', label: 'Friso Horizontal',
          largura: ctx => Math.min(ctx.larguraQuadro2F + 50, 1480),
          comp: ctx => ctx.eF + 100,
          ext: 1, int: 1, categoria: 'porta' },
        { id: 'friso_horizontal_2', label: 'Friso Horizontal 2',
          largura: ctx => Math.max(0, (ctx.larguraQuadro2F + 50) - 1480),
          comp: ctx => ctx.eF + 100,
          ext: ctx => (ctx.larguraQuadro2F + 50) > 1480 ? 1 : 0,
          int: ctx => (ctx.larguraQuadro2F + 50) > 1480 ? 1 : 0, categoria: 'porta' },
        { id: 'friso_vertical', label: 'Friso Vertical',
          largura: ctx => ctx.eF + 100,
          comp: ctx => ctx.alturaQuadro,
          ext: ctx => (ctx.larguraQuadro2F + 50) > 1480 ? 1 : 0,
    ...
    26: {
      ...
      '1F': [
        // Felipe: VIDRO CENTRAL nao e' mais peca de chapa aqui — ele ja' e'
        // custeado por m2 (dimensoesBaseMod26 em computeRevestimentoPorCor).
        // Sair como peca ACM na tabela era redundante -> removido.
        { id: 'acm_moldura_lat1', label: 'Moldura ACM Altura 1',
          largura: ctx => (_molduraMedidaMod26(ctx) != null ? _molduraMedidaMod26(ctx) : 148) + 2*ctx.REF,
          comp:    ctx => _dimBaseMod26(ctx).paf,
          ext: 2, int: 2, categoria: 'porta' },
        { id: 'acm_moldura_lat2', label: 'Moldura ACM Altura 2',
          largura: ctx => { var M=_molduraMedidaMod26(ctx); return (M!=null ? M-38 : 110) + 2*ctx.REF; },
          comp:    ctx => _dimBaseMod26(ctx).paf,
          ext: 2, int: 2, categoria: 'porta' },
        { id: 'acm_moldura_h', label: 'Cantoneira o vidro 1',
          largura: ctx => { var M=_molduraMedidaMod26(ctx); return (M!=null ? M-28 : 124) + ctx.REF; },
          comp:    ctx => _dimBaseMod26(ctx).travHorizontal,
          ext: 4, int: 4, categoria: 'porta' },
        { id: 'acm_moldura_v', label: 'Cantoneira o vidro 2',
          largura: ctx => { var M=_molduraMedidaMod26(ctx); return (M!=null ? M-28 : 124) + ctx.REF; },
          comp:    ctx => _dimBaseMod26(ctx).travVertical,
          ext: 4, int: 4, categoria: 'porta' },
        { id: 'acm_moldura_f161', label: 'Moldura ACM Largura 1',
