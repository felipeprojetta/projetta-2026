...
/* ── Config loader (cacheia em window._FRETE_CFG) ──────────────────────── */
...
function calcFrete(input, cfg){
  ...
  if(rota.modal==='LCL'){
    ...
    if(input.dap>0){
      sec('DAP (entrega no destino)');
      au('DAP Charges', input.dap);
    }
  ...
  if(iofBrl||issBrl){
    ...
    if(issBrl){
      lines.push({desc:'ISS (reportado — não soma no total)', moeda:'BRL', valor:issBrl, brl:issBrl, noSum:true});
    }
  }

  var totalBRL=usdSum*cambio + brlDir + iofBrl;
  var totalUSDEquiv=totalBRL/cambio;

  return {
    lines:lines, usdSum:usdSum, brlDirect:brlDir, iof:iofBrl, iss:issBrl,
    totalBRL:totalBRL, totalUSDEquiv:totalUSDEquiv,
...
function buildRotaOptions(cfg){
  ...
  cfg.frete_rotas_v1.rotas.forEach(function(r){
    var lbl=r.destino_nome+' ('+r.destino_pais+') · '+r.modal+' · '+
            (r.modal==='LCL'?('US$ '+r.usd_cbm+'/CBM'):('US$ '+r.usd_flat+' flat'));
    var o=document.createElement('option');
    o.value=r.id; o.textContent=lbl;
    sel.appendChild(o);
  });
}
...
function renderBreakdown(r){
  ...
  r.lines.forEach(function(l){
    if(l.section){
      ...
      html+='<tr>'
        +'<td style="padding:5px 8px;color:#555;font-size:11px">'+escH(l.desc)+'</td>'
        +'<td style="padding:5px 8px;text-align:right;color:#888;font-size:10px;font-variant-numeric:tabular-nums">'+origin+'</td>'
        +'<td style="padding:5px 8px;text-align:right;color:#888;font-size:10px">'+mult+'</td>'
        +'<td style="padding:5px 8px;text-align:right;font-variant-numeric:tabular-nums;font-size:11px">'+brlDisplay+'</td>'
        +'</tr>';
    }
  });
  body.innerHTML=html;
...
function onRotaChange(){
  ...
  if(!rota) return;
  /* Pré-preenche DAP sugerido e usd_cbm da rota */
...
window.freteOpenCalc=function(){
  loadConfig(function(cfg){
    if(!cfg){
      alert('Erro: não foi possível carregar a configuração de frete da nuvem.\nVerifique conexão ou avise o admin.');
      return;
    }
    buildRotaOptions(cfg);

    /* Pré-preenche CBM da caixa, câmbio do campo de instalação internacional */
    ...
    /* Restaura último cálculo se existir (por card) */
...
window.freteAplicar=function(){
  ...
  /* Converte o total BRL de volta pra USD no câmbio escolhido, pra caber no
     campo crm-o-cif-frete-maritimo que é em USD */
  ...
  /* Salva breakdown em memória e localStorage */
  window._freteCalc=r;
  try{
    var cardId=window._crmOrcCardId||'_novo';
    var inp=currentInput();
  ...
  /* Gatilho do recalc CIF existente */
  if(typeof window.crmCifRecalc==='function') window.crmCifRecalc();

  window.freteCloseCalc();
};

/* ── Wire-up dos inputs do modal ───────────────────────────────────────── */
function wireModalInputs(){
  var ids=['frete-calc-rota','frete-calc-cbm','frete-calc-peso','frete-calc-cambio',
           'frete-calc-dap','frete-calc-oversize','frete-calc-iof-on'];
...
function init(){
  wireModalInputs();
  /* Pré-carrega config em background pra abrir modal rápido na 1ª vez */
  loadConfig(function(cfg){
