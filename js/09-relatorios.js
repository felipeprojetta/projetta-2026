...
function relatorioBarras(){
  ...
  d.seenKeys.forEach(function(key){
    ...
    groups[forn].push({code:key,desc:perfil?perfil.d:key,qty:r.nBars,barra:r.barLenMM||6000,kgBruto:r.kgBruto||0,precoKg:r.precoKg||0,custo:r.custoTotal||0,pintado:r.pintado,precoKgPint:r.pintado?precoPintLiq:0,_isBoiserie:!!r._isBoiserie,_barPrice:r._barPrice||0});
  });
  var h='';
  ...
  for(var forn in groups){
    ...
    items.forEach(function(it){
      ...
      var obs=it._isBoiserie?'R$150/barra':(it.pintado?'+ R$ '+(it.precoKgPint||0).toFixed(2)+'/kg pint.':'');
      var precoCol=it._isBoiserie?'R$ 150/barra':'R$ '+(it.precoKg||0).toFixed(2);
      h+='<tr><td style="padding:4px 8px;border:0.5px solid #eee;text-align:center;color:#888">'+n+'</td><td style="padding:4px 8px;border:0.5px solid #eee;font-weight:600;color:#003144">'+it.code+'</td><td style="padding:4px 8px;border:0.5px solid #eee;text-align:center">'+it.qty+'</td><td style="padding:4px 8px;border:0.5px solid #eee;text-align:center">'+it.barra+'</td><td style="padding:4px 8px;border:0.5px solid #eee;text-align:right">'+it.kgBruto.toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2})+'</td><td style="padding:4px 8px;border:0.5px solid #eee;text-align:right;color:#888">'+precoCol+'</td><td style="padding:4px 8px;border:0.5px solid #eee;text-align:right;font-weight:700;color:#003144">R$ '+it.custo.toLocaleString('pt-BR',{minimumFractionDigits:2})+'</td><td style="padding:4px 8px;border:0.5px solid #eee;font-size:9px;color:#888">'+obs+'</td></tr>';
    });
