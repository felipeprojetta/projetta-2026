(() => {
  if (window.__MOD_131_V4_LOADED) return;
  window.__MOD_131_V4_LOADED = true;
  ...
  // ── 1. Bloquear auto-fetch + manual only ────────────────
  whenReady(function(){
    try {
      ...
      console.log('[mod 131 v4] manual only ATIVADO — atual: ' + window.projettaCambio.get());
    } catch(e){}
  });
  ...
  function propagarCambio(valor) {
    ...
    // Atualiza TODOS os campos com dispatch
    ...
    // Recalcular tudo
    ['crmCifRecalc', 'crmCifRecalcBRL', 'calc', 'crmRender', 'recomputeAndRender', 'calcAll', 'populateProposta'].forEach(function(fn){
      if (typeof window[fn] === 'function') {
        try { window[fn](); } catch(e){ console.warn('[mod 131 v4] erro em '+fn+':', e); }
      }
    });

    console.log('[mod 131 v4] cambio propagado: ' + valor);
  }

  // ── 3. Hook em populateProposta — sempre forca cambio antes de renderizar
  function hookPopulateProposta() {
    if (typeof window.populateProposta !== 'function') return false;
    if (window.populateProposta.__mod131v4) return true;
    var orig = window.populateProposta;
    window.populateProposta = function() {
      try {
        // Forca inst-intl-cambio com valor master ANTES da proposta ler
        ...
        if (v) {
          ...
          // Tambem tenta cambio-master-input
          var master = document.getElementById('cambio-master-input');
          if (master) master.value = v.toFixed(4);
        }
      } catch(e){}
      return orig.apply(this, arguments);
    };
    window.populateProposta.__mod131v4 = true;
    console.log('[mod 131 v4] populateProposta hooked — sempre usa cambio master');
    return true;
  }

  // ── 4. Hook em crmCifRecalc — idem
  function hookCifRecalc() {
    if (typeof window.crmCifRecalc !== 'function') return false;
    if (window.crmCifRecalc.__mod131v4) return true;
    ...
    window.crmCifRecalc.__mod131v4 = true;
    console.log('[mod 131 v4] crmCifRecalc hooked');
    return true;
  }

  // ── 5. Injetar campo no modal CRM ────────────────────────
  function injetarCampoCambio() {
    ...
    bloco.innerHTML =
      ...
      '<div style="font-size:10px;color:#7a3e00;margin-bottom:8px;font-style:italic;line-height:1.4">Esse valor vai pra Orcamento, Frete, Proposta, Caixa e todos os outros lugares.</div>' +
    ...
    var propagar = function(){
      ...
      propagarCambio(v);
      if (status) {
        status.textContent = '✓ aplicado em todos os lugares';
    ...
    console.log('[mod 131 v4] campo cambio injetado no modal');
  }

  // Tentar instalar tudo periodicamente
  ...
  console.log('[mod 131 v4] carregado — hooks em populateProposta + crmCifRecalc + propaga em todos os campos');
})();
