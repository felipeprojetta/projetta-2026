...
function _populatePropostaItens(){
  ...
  //   EXW → nenhuma linha (cliente coleta na fabrica)
  ...
  if(_cifData && _ISINTL){
    ...
    // Linha CAIXA (FOB + CIF — se tem volume > 0)
    if(_cifData.incluirCaixa && _cifData.caixaUSD > 0){
    ...
    // Linha FRETE TERRESTRE (FOB + CIF)
    if(_cifData.incluirTerrestre && _cifData.freteTerrestreUSD > 0){
    ...
    // Linha FRETE MARÍTIMO (só CIF)
    if(_cifData.incluirMaritimo && _cifData.freteMaritimoUSD > 0){
